...
### 1. 押注与欠债
- **欠债限制**：最大欠款为初始资金 (20) 的 10 倍，即 -200  
- **欠债时机**：只能在主菜单通过"借钱"按钮产生欠债，游戏中不允许欠钱  
- **服务器校验**：房主必须校验所有投注，确保 `betAmount ≤ player.currentMoney`  
- **赢钱清债**：赢得底池后，优先清偿欠债，剩余才计入账户  

### 2. 弃牌规则
- **每次弃牌消耗 1 次弃牌次数**（默认 3 次），与出牌次数无关  
- **立即补牌**：弃牌后立即从牌组抽取等数量新牌，弃掉的牌随机插回牌组  
- **多次弃牌**：玩家可在一个回合内多次弃牌直到耗尽弃牌次数  

### 3. 出牌与观战
- **一局一次**：每个玩家整局只能出牌一次（出牌次数 = 1）  
- **观战模式**：出过牌后的玩家在未来回合直接跳过，进入观战状态  
- **手牌处理**：出牌后，所有手牌（含出牌的）随机插入牌组  

### 4. 小丑牌触发顺序
- **手动排序**：玩家可在主菜单与游戏中调整小丑牌顺序  
- **从左到右触发**：核算时小丑牌依次从左到右检测触发条件，依次触发  

### 5. 商店与持久化
- **本地刷新**：每局结束返回主菜单时，随机刷新商店（10 张小丑牌）  
- **不补货**：当局内购买过后不补货，直到下一次刷新  
- **JSON 存储**：玩家档案采用 JSON 格式，字段包括 `playerName`, `avatarId`, `currentMoney`, `debt`, `jokersList`, `borrowedAmount`  
